            ...
            <table class="table table-hover" id="qna">
                ...
                <c:forEach items="${ list }" var="dto" varStatus="idx" begin="${ page.startIdx }" end="${ page.endIdx }" >
                    <tr>
                        ...
                        <td scope="row"><a href='/admin/adminPdUpdate?pd_id=${ dto.pd_id }&pd_size=${ dto.pd_size }' id="id">${ dto.pd_id }</a></td>
                        <td scope="row"><a href='/admin/adminPdUpdate?pd_id=${ dto.pd_id }&pd_size=${ dto.pd_size }'>${ dto.pd_name }</a></td>
                        <td>${ dto.pd_color }</td>
            ...
            <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                <div class="btn-group mr-2" role="group" aria-label="First group">
                		<c:if test="${page.startPageIdx > 1}">
                   			<button type="button" class="btn btn-secondary" onclick="location.href='/admin/adminPdSearch?page=${page.thisPage-1}&search=${search }' ">prev</button>
                   		</c:if>
                 <c:forEach varStatus="num" begin="${page.startPageIdx}" end="${ page.endPageIdx }" >
		         	<button type="button" class="btn btn-secondary" onclick="location.href='/admin/adminPdSearch?page=${num.index}&search=${search }' ">${num.index}</button>
		         </c:forEach>
                   <c:if test="${page.totalPage > page.endPageIdx}">
                   		<button type="button" class="btn btn-secondary" onclick="location.href='/admin/adminPdSearch?page=${page.thisPage+1}&search=${search }' ">next</button>
                   </c:if>
                </div>
            </div>
        </div>
